...
%   Load = -5*ones(size(LowerHalfCircleNodes,1),3)./n;
%   Load(:,1) = LowerHalfCircleNodes; Load(:,2) = 0;
  NStep = 100; % Number of time steps
  Th = linspace(0,0.5*pi,NStep+1);
  Load = zeros(n,2*(NStep+1)+1); Load(:,1) = LowerHalfCircleNodes;
  Load(:,3:2:end) = -1e2*sin(Th).*ones(n,1)./n; % Half-cycle sinusoidal load
%   Load(:,3:2:end) = -1e2*repmat(1+rand(1,NStep+1),n,1)./n; 
  x = {Supp,Load};
