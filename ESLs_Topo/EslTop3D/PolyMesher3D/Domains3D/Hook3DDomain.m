...
%     -80.6226).^2)-10)<0.04 & Node(:,2)>+80.6226 & abs(Node(:,3))<eps);
  UpperHalfCircleNodes = find(abs(sqrt(Node(:,1).^2+(Node(:,2)...
    -80.6226).^2)-10)<eps & Node(:,2)>+80.6226);
...
%       Node(:,2)<0);
  LowerHalfCircleNodes = ...
      find(abs(sqrt(Node(:,1).^2+Node(:,2).^2)-19)<eps & ...
      Node(:,2)<0);
  % Static Load
...
%   Load(:,1) = LowerHalfCircleNodes; Load(:,2) = 0; Load(:,4) = 0;
  ...
  n = length(LowerHalfCircleNodes);
  Th = linspace(0,pi,NStep+1);
  Load = zeros(n,3*(NStep+1)+1);
  Load(:,1) = LowerHalfCircleNodes; Load(:,2) = 0; Load(:,4) = 0;
  %Load(:,3:3:end) = -1e2*sin(Th).*ones(n,1)/n; % Half-cycle sinusoidal load
  % Random load
%   RLoad = 1e2.*rand(1,NStep+1);
%   RLoad = 1e2.*ones(1,NStep+1);
%   plot(RLoad);
%   save('Result\Case1\RLoad.mat','RLoad');
  load('Result\Case1\RLoad.mat','RLoad');
%   Load(:,3:3:end) = -repmat(RLoad,n,1)/n;
  % Moving random load
  Lt = 7.6; % Moving load length
  L = 38; % Total length
  for ii=1:NStep+1
    x = L*(ii-1)/NStep-L/2;
    MovLoad = find((Node(LowerHalfCircleNodes,1)>=x-Lt)& ...
        (Node(LowerHalfCircleNodes,1)<=x));
    NLoad = length(MovLoad);
    Load(MovLoad,3*ii) = -repmat(RLoad(ii),NLoad,1)/n;
  end
  x = {Supp,Load};
