...
%   Load = -1e3*ones(size(LeftHalfCircleNodes,1),4);
%   Load(:,1) = LeftHalfCircleNodes; Load(:,2) = 0; Load(:,4) = 0;
  n = length(LeftHalfCircleNodes);
  NStep = 200; % Number of time steps
  Th = linspace(0,pi,NStep+1);
  Load = zeros(n,3*(NStep+1)+1); Load(:,1) = LeftHalfCircleNodes;
  Load(:,3:3:end) = -sin(Th).*ones(n,1);  
  %Load(:,3:3:end) = -repmat(rand(1,NStep+1),n,1)/n;
  x = {Supp,Load};
